            ...
            if (msg.message['is_echo'] === true) {
              ...
              if (msg.message.sticker_id) {
                if (msg.message.sticker_id === 369239263222822) {
                  sendSimpleTextMessage(msg.sender.id, "따봉 고마워");
                  setTimeout(() => {
                    sendSimpleTextMessage(msg.sender.id, "근데 큰 따봉 줄 순 없었니?");
                  }, 1000);
                } else if (msg.message.sticker_id === 369239343222814 || msg.message.sticker_id === 369239383222810) {
                  sendSimpleTextMessage(msg.sender.id, "큰 따봉 고마워");
                  setTimeout(() => {
                    sendSimpleTextMessage(msg.sender.id, "근데 난 너가 공부하는게 더 좋아");
                  }, 1000);
                  setTimeout(() => {
                    sendSimpleTextMessage(msg.sender.id, "미안.");
                  }, 2000);
                } else {
                ...
                sendSimpleTextMessage(msg.sender.id, "너가");
                setTimeout(() => {
                  sendSimpleTextMessage(msg.sender.id, "'공부'라고 하면 단어 공부를 시작해");
                }, 1000);
                setTimeout(() => {
                  sendSimpleTextMessage(msg.sender.id, "'힌트'라고 하면 마지막 문제에 대한 힌트를 줘");
                }, 2000);
                setTimeout(() => {
                  sendSimpleTextMessage(msg.sender.id, "'복습'이라고 하면 지금까지 틀린 단어들 복습을 시작해");
                }, 3000);
                setTimeout(() => {
                  sendSimpleTextMessage(msg.sender.id, "'진도'라고 하면 현재 상황을 보여줘");
                }, 4000);
                setTimeout(() => {
                  sendSimpleTextMessage(msg.sender.id, "'추가기능'이라고 하면 또 다른 기능들을 알려줄께");
                }, 5000);
                setTimeout(() => {
                  sendSimpleTextMessage(msg.sender.id, "아참, 난 초성으로만 써도 알아들어^^");
                }, 5000);
                ...
                docClient.get(params, function(err, data) {
                  if (err) {
                    ...
                    setTimeout(() => {
                      sendSimpleTextMessage(msg.sender.id, '복습 하고 갈래? *^^*');
                    }, 10000);
                  }
                })
...
function sendQuestionMessage(recipientId, messageText) {
  var messageData = {
    recipient: {
      id: recipientId
    },
    message: questionGenerator()
  };
  var questionWord = messageData.message.attachment.payload.text.split('의')[0];
...
function questionGenerator(word) {
...
// console.log(JSON.stringify(questionGenerator()));

// console.log(getHierarchy('local'))
